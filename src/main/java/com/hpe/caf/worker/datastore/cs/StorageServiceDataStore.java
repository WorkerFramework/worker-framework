...
public class StorageServiceDataStore implements ManagedDataStore
{
    ...
    private final int FILE_THRESHOLD = 1024;
    ...
    public StorageServiceDataStore(final StorageServiceDataStoreConfiguration storageServiceDataStoreConfiguration){
        storageClient = new StorageClient(storageServiceDataStoreConfiguration.getServerName(),
                String.valueOf(storageServiceDataStoreConfiguration.getPort()), storageServiceDataStoreConfiguration.getEmailAddress());
    ...
    public InputStream retrieve(String reference) throws DataStoreException {
        ...
        StorageServiceReference storageServiceReference = getStorageServiceReference(reference);
        AssetMetadata assetMetadata = getAssetMetadata(storageServiceReference);
        if(AssetStatus.ACTIVE.equals(AssetStatus.valueOf(assetMetadata.getStatus()))) {
            ...
            try {
                Asset asset = this.storageClient.downloadAsset(storageServiceReference.getContainerId(), storageServiceReference.getAssetId(), wrappedKey);
                return asset.getDecryptedStream();
            } catch (IOException e) {
    ...
    private StorageServiceReference getStorageServiceReference(String reference) throws DataStoreException {
        Objects.requireNonNull(reference);
    ...
    private AssetMetadata getAssetMetadata(StorageServiceReference storageServiceReference) {
        Objects.requireNonNull(storageServiceReference);
    ...
    public long size(String reference) throws DataStoreException {
        ...
        return assetMetadata.getSize();
    }

    @Override
    public String store(InputStream inputStream, String partialReference) throws DataStoreException {
        try{
            FileBackedOutputStream fileBackedOutputStream = new FileBackedOutputStream(FILE_THRESHOLD, true);
            try {
    ...
    @Override
    public String store(Path path, String partialReference) throws DataStoreException {
    ...
    private String store(ByteSource byteSource, String partialReference) throws DataStoreException{
        ...
        try(InputStream inputStream = byteSource.openBufferedStream()){
            AssetMetadata assetMetadata = this.storageClient.uploadAsset(storageServiceReference.getContainerId(),
                    wrappedKey, assetKey, inputStream, null,
    ...
    private void verifyPartialReference(StorageServiceReference storageServiceReference) throws DataStoreException{
