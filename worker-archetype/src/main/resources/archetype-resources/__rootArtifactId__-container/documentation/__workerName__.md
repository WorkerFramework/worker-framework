...
This code includes an implementation of a simple ${workerName} for the CAF Worker framework. It performs
a text conversion on a text file based on an `Action` enumeration passed in from the `${workerName}Task` class. It retrieves
...
The configuration class is `${workerName}Configuration`, which has several options:

- `workerVersion`: the version number of this Worker
...
The task class is `${workerName}Task` and has the following entries:
...
The result class is `${workerName}Result` and has the following entries:
...
Status can have one of the following values:
    ...
    - `WORKER_FAILED`: the input file could be read but the worker failed in an unexpected way
- `textData`: a reference to the result data in the DataStore or the data itself.


## Health checks

This Worker provides a basic health check. It creates an `${workerName}FactoryProvider` object using the `ModuleLoader`.
...
`${workerName}Configuration` member `threads`.

Memory usage will vary significantly with the size of the input file.

Any result whose size exceeds the `resultSizeThreshold` stipulated in the configuration class `${workerName}Configuration`
will be written to the DataStore rather than being held in a byte array directly within the worker result.


## Failure modes

The main points of failure for `${workerName}` are:
...
- `IO errors`: for results with `WORKER_FAILED`, this could be caused by the InputStream to String conversion, and
...
of `${workerName}-shared` has not changed then an upgrade to `${workerName}` is an in-place upgrade.

If you need to do a rolling upgrade when `${workerName}-shared` has changed
then:

- Spin up containers of the new version of `${workerName}`
- Replace old versions of producers of `${workerName}Task` with new ones
- Allow the queue with the old versions of `${workerName}Task` to drain and
